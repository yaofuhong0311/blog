...
description: 一轮源码级的沙箱引擎调研沉淀：别背参数表，从七个角度提问；隔离的本质是四种哲学；引擎在商品化，价值在 facade。
...
每个角度上不同引擎给出的答案差异巨大，而且**角度之间会打架**——后面会看到"持久卷和 warm pool 的矛盾"（存储 vs 启动）、"强隔离和 K8s 原生的矛盾"（隔离 vs 调度）。选型的实质是决定在哪几个角度上让步。
...
另外两个常被忽略的运营坑：Firecracker **没有 live migration**，节点要下线只能杀掉重来；**快照是 host-bound 的**（绑定宿主机的 CPU 特性和内存布局），不能随便搬到异构节点上恢复。这些都不是性能参数，但决定了你的运维模型。
...
更有意思的是各引擎在这一层的差异其实**不影响结局**：gVisor 用自研用户态协议栈（netstack）在自己层里拦，microVM 走 virtio-net + TAP 把包全甩给宿主机、hypervisor 层零过滤——但无论哪种，**域名级 egress 控制最终都收敛到宿主机层执行**。也就是说这个能力和选哪个引擎基本正交，别指望引擎替你解决。顺带一个陷阱：基于 DNS 解析结果做放行是有 TOCTOU 窗口的（DNS rebinding——校验时解析到合法 IP，连接时已经换了），设计防线时要意识到"检查"和"使用"之间隔着时间。
...
文件共享方案里，virtio-fs（FUSE + DAX，宿主机页缓存直接映射进 guest）性能远好于话痨的 virtio-9p；而 Kata 的实际架构是"guest 系统镜像走 virtio-blk、容器内容走 virtio-fs"两条独立路径——调研前我以为的"qcow2 vs virtio-fs 二选一"根本是个伪命题。
...
把公开的几个主流开源引擎放到这七个角度上跑完（Firecracker 系的 E2B、RustVMM 系的 CubeSandbox、K8s Operator 路线的 OpenSandbox），我的结论是：

**引擎本身在快速商品化。** 冷启动都在卷毫秒级，snapshot 都在做，SDK 接口在向事实标准收敛（E2B 兼容成了新引擎的标配卖点）。单看引擎能力，差距在缩小。

**真正拉开差距的是运营模型。** microVM 路线普遍刻意不用 K8s——特权、宿主内核依赖、本地有状态盘、毫秒级调度，正是 K8s 的盲区，硬塞进去是逆设计；容器路线 K8s 原生、零新增基建，但隔离强度和持久化有天花板。**选引擎实质上是在选运营模型**：要不要为它立专用节点池？团队愿不愿意运维第二套编排系统？

所以架构上的正解是**把宝押在 facade 层**：对上暴露统一的沙箱 API（隔离强度、要不要持久卷做成路由参数），对下把引擎当可替换的 provider。引擎会换代，接缝是长期资产——这和上一篇 [AI Coding 工作流](/posts/ai-coding-workflow-2026/) 里"边界测试套件是长期资产"是同一个道理：**易变的部分后面，永远站着一个值得投资的稳定接口**。

---

最后说一句方法：这轮调研全程是和 AI 结对完成的——每个角度先让它做源码级调研，再起独立的子代理做对抗式核验（专门找第一轮结论的漏洞），错的当场纠正再回填文档。比自己啃源码快得多，比直接问 AI 可靠得多。这套打法本身，值得再写一篇。
